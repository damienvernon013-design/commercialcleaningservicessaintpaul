// Vercel serverless function — proxies quote-wizard bookings to the CRM-QM PushLead API.
...
const ZIP_DEFAULT = '55101'; // Saint Paul, MN
const ADDRESS_DEFAULT = 'Saint Paul, MN';
// CRM_API_TOKEN is shared across the whole portfolio (60+ sites) with no
// separate site-id field in the PushLead schema, so this is the only
// reliable way to trace a lead back to this specific domain. Keep this in
// sync with the site's actual domain if it's ever renamed.
const SITE_SOURCE_TAG = 'Site: commercialcleaningservicessaintpaul.com';
  ...
  return {
    value: {
      ...
      customer: {
        ...
        notes: [SITE_SOURCE_TAG, clean(input.notes)].filter(Boolean).join(' | '),
      },
      industry,
